...
export const PAYMENT_STATUS_TAGS: Record<string, { label: string; classes: string }> = {
  awaiting_payment: { label: 'Payment Due', classes: 'bg-amber-100 text-amber-800' },
...
export const COLLECTION_MODE_TAGS: Record<string, { label: string; classes: string }> = {
  self_pickup: { label: 'Self Pickup', classes: 'bg-sky-100 text-sky-700' },
  courier: { label: 'Courier', classes: 'bg-violet-100 text-violet-700' },
};
